...
(function() {
    ...
    var CustomInlines = (function() {
        ...
        function CustomInlines(item, options) {
            ...
            this.totalForms = this.data.querySelector('#id_' + this.options.prefix + '-TOTAL_FORMS')
            this.maxForms = this.data.querySelector('#id_' + this.options.prefix + '-MAX_NUM_FORMS')
        ...
        CustomInlines.prototype = {
            add(event) {
                ...
                window.Select2.prototype.run2(() => {
                    const selects = document.querySelectorAll('.select2-container');
                
                   selects.forEach(item => item.remove())
                })
                popupClass.prototype.initLinks()
            },
